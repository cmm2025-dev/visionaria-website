...
/**
 * One-time admin flow to (re-)mint the long-lived refresh token the backend uses to read Desk
 * data on behalf of all clients. Visit this route once (with ?key=ZOHO_SETUP_SECRET) as a Zoho
 * admin, approve the scopes, and copy the refresh_token shown into ZOHO_SERVICE_REFRESH_TOKEN in
 * Vercel. Delete this route again once done.
 */
export async function GET(req: NextRequest) {
  ...
  if (!code) {
    ...
    const params = new URLSearchParams({
      response_type: 'code',
      client_id: clientId,
      scope: 'Desk.tickets.READ,Desk.contacts.READ,Desk.basic.READ,Desk.search.READ,Desk.customModule.cm_repositorio_documentalALL',
