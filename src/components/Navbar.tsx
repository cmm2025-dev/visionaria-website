...
export default function Navbar({ locale, t }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b backdrop-blur-md" style={{ background: 'rgba(30,27,24,0.90)', borderColor: 'var(--border)' }}>
      ...
      {open && (
        <div className="lg:hidden border-t px-4 pb-4" style={{ borderColor: 'var(--border)', background: 'rgba(30,27,24,0.98)' }}>
          {links.map(({ href, key }) => (
            <Link
              key={key}
              href={`/${locale}${href}`}
